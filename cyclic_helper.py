...
class CyclicHelper:
    def __init__(self):
        ...
        self.cyclic_y_pid = PIDCalculator(max_auth=self.max_auth, learning_rate=self.learning_rate * 50)
    ...
    def update(self, Vx, Vy, Vz, Pitch, Roll, Yaw, PitchRate, RollRate, blocked = False, manual_cyclic_x=0.0, manual_cyclic_y=0.0, hovering=False):
        ...
        if hovering:
            ...
            pre_error_y = min(max(-0.05*(self.forward) - 0.085, -0.15), 0.15)
        else:
            pre_error_acc_up = 0.0001 * acc_up * min(max(Pitch, -1), 1)

        # 記錄 pitch 歷史
        if abs(manual_cyclic_y) > 0.02 and not hovering and abs(manual_cyclic_x) < 0.05:
